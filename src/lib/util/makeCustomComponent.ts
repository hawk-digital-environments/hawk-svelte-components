...
export function makeCustomComponent(baseClass: new () => HTMLElement): new () => HTMLElement {
    return class CustomComponent extends baseClass {
        private readonly _childStorage: HTMLSpanElement;
        private readonly _snippetStorage: Record<string, HTMLSpanElement> = {};

        constructor() {
            super();
            this._childStorage = document.createElement('span');
            this._childStorage.append(...this.childNodes);

            // Extract all snippets into their own dedicated storage
            const snippets = this._childStorage.getElementsByTagName('hawk-snippet');
            for (const snippet of snippets) {
                const name = snippet.getAttribute('property') || '';
                if (name) {
                    if (!this._snippetStorage[name]) {
                        this._snippetStorage[name] = document.createElement('span');
                        (this as any)[name] = propertySnippetFactory(this._snippetStorage[name]);
                    }
                    this._snippetStorage[name].append(...snippet.childNodes);
                } else {
        ...
        get children() {
            const mixedSnippet = propertySnippetFactory(this._childStorage) as Snippet & HTMLCollection;
...
function propertySnippetFactory(storage: HTMLSpanElement): Snippet {
